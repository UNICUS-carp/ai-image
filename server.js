...
2. 各小見出しに対応する本文から、視覚的に表現しやすい具体的な描写を抽出
3. 小見出しの順序を保持
4. 小見出しが6つ以上ある場合のみ、最も重要な5つを選択してください
5. 抽象的な説明より、具体的な動作・場面・物体の描写を優先
...
- 見出しごとに本文を適切に分割`;
    ...
    // 小見出しなしの場合
    prompt = `あなたは日本語の文章を分析し、画像生成に適した部分を抽出するエキスパートです。

【タスク】
以下の本文から、視覚的に表現しやすい具体的な場面や描写を含む部分を抽出し、分割してください。

【本文】
${content}

【抽出の優先順位】
1. 具体的な動作・行動の描写（例：歩く、座る、手を挙げる）
2. 視覚的な場面・情景（例：公園、オフィス、キッチン）
3. 人物や物体の具体的な描写（例：笑顔の女性、赤い花）
4. 感情や状態が視覚的に表現できる部分（例：喜ぶ、疲れている）
5. 比喩や例え話で視覚化できる部分

【要件】
1. 要約せず、原文から視覚的に表現可能な部分をそのまま抽出
2. 各チャンクは100-400字（視覚的要素を優先）
3. 最大5チャンクまで
4. 抽象的な説明や理論的な部分は避ける
5. 全体が200字未満なら最も視覚的な部分を1チャンク
...
必ずJSON形式で出力してください：
{
  "chunks": [
    {
      "text": "抽出した視覚的に表現可能な本文",
      "charCount": 245,
      "visualElements": "この部分の主要な視覚要素（例：人物、場所、動作）"
    }
  ]
}
